...
const Sidebar = () => {
  ...
  useEffect(() => {
    // Load active tab index from localStorage on component mount
  ...
  const handleAccountClick = () => {
    // Set active link index to the Overview tab
    setActiveLink(0);
  ...
  if (isLoginPage) {
    return null; // Do not render the sidebar on the login page
  }
      ...
      <div className='flex items-center space-x-6 p-4 w-2/3 absolute bottom-4 left-4 px-1 py-2'>
        <div className='flex items-center cursor-pointer w-full' onClick={handleAccountClick}>
          ...
          <div className='flex-1 justify-between items-center leading-5 w-52 ml-3'>
            ...
            <span className='text-xs text-gray-600'>Receptionist</span>
          </div>
        </div>
